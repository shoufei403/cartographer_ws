...
int MapBuilderBridge::AddTrajectory(//这个是主要执行函数 添加一个Trajectory,然后得到一个trajectory_id.并且相应地配对一个sensor_bridges
  ...
  const int trajectory_id = map_builder_->AddTrajectoryBuilder(
      ...
      ::std::bind(&MapBuilderBridge::OnLocalSlamResult, this, 
                  ...
                  ::std::placeholders::_5));/*传入了一个激光数据scanmatcher的回调函数*/
...
void MapBuilderBridge::OnLocalSlamResult(
  ...
  cartographer::common::MutexLocker lock(&mutex_);
  trajectory_state_data_[trajectory_id] = std::move(local_slam_data);//这里保存了前端匹配的结果
}
