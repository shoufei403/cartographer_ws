...
void SensorBridge::HandleImuMessage(const std::string& sensor_id,
  ...
  if (imu_data != nullptr) {
    trajectory_builder_->AddSensorData(  //添加IMU数据
        sensor_id,
...
void SensorBridge::HandleRangefinder(
  ...
  if (sensor_to_tracking != nullptr) {
    trajectory_builder_->AddSensorData(     //HandleLaserScanMessage -> HandleLaserScan -> HandleRangefinder 
        sensor_id, carto::sensor::TimedPointCloudData{ //激光数据被整理成合适的数据格式后传入到cartographer
