...
{
  //设置搜索范围.
  const SearchParameters search_parameters(options_.linear_search_window(),//可设置的局部搜索范围
                                           options_.angular_search_window(),
                                           point_cloud, limits_.resolution());

  //然后进行匹配.
  return MatchWithSearchParameters(search_parameters, initial_pose_estimate, //给定了初始位置
    ...
    transform::Rigid2d* pose_estimate) const
{
  ...
  return MatchWithSearchParameters(search_parameters, center, point_cloud, //这里使用地图中心作为初始位置
...
{
  //计算解的数量.
  const int linear_step_size = 1 << precomputation_grid_stack_->max_depth();//对于real time CSM，这里的linear_step_size一直为1
